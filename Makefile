...
test-endpoints:
	...
	@ssh -i $(SSH_KEY) $(SSH_USER)@$(SSH_HOST) '\
		...
		echo ""; \
		echo "Producción:"; \
		curl -s https://back-chat.converxa.net/api/health | jq . || echo "❌ Error o no JSON válido"; \
		echo ""; \
		echo "Pruebas internas:"; \
		curl -s https://internal-back-chat.converxa.net/api/health | jq . || echo "❌ Error o no JSON válido"'
...
urls:
	...
	@echo "  https://back-chat.converxa.net/api/health"
	@echo ""
	...
	@echo "  https://internal-back-chat.converxa.net/api/health"
	@echo ""
	@echo "$(YELLOW)SSH:$(NC)"
