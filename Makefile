...
health:
	...
	@ssh -i $(SSH_KEY) $(SSH_USER)@$(SSH_HOST) '\
		...
		echo -n "Producción: "; \
		if curl -sf https://dev-converxa.converxa.net/api/health >/dev/null 2>&1; then
			echo "✅ HEALTHY"; \
		else \
			echo "❌ UNHEALTHY"; \
		fi'
...
test-endpoints:
	...
	@ssh -i $(SSH_KEY) $(SSH_USER)@$(SSH_HOST) '\
		...
		echo ""; \
		echo "Producción:"; \
		curl -s https://dev-converxa.converxa.net/api/health | jq . || echo "❌ Error o no JSON válido"; \
		echo ""; \
		echo "Pruebas internas:"; \
		curl -s https://internal-dev-converxa.converxa.net/api/health | jq . || echo "❌ Error o no JSON válido"'
...
urls:
	...
	@echo "  https://dev-converxa.converxa.net/api/health"
	@echo ""
	...
	@echo "  https://internal-dev-converxa.converxa.net/api/health"
	@echo ""
	@echo "$(YELLOW)SSH:$(NC)"
